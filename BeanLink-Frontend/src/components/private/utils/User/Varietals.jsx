...
export const Varietals = () => {
  ...
  useEffect(() => {
    if (auth.coffeeVarieties) {
      setVarietales(auth.coffeeVarieties);
    }
  }, []);
  ...
  const addItem = () => {
    ...
    setVarietales((prev) => [...prev, { name: "", isNew: true }]);
  };
  ...
  const deleteItem = (index) => {
    setVarietales((prev) => {
      ...
      let varietalesCopied = [...prev];
      //Elimimaoms el registro correspondiente al indice que nos llega
      let varietalesDeleted = varietalesCopied.filter((_, i) => i !== index);
      //Devolvemos para setear el estado
      return varietalesDeleted;
    });
  };
  ...
  return (
    <div>
      ...
      {varietales.map((varietal, index) => {
        return (
          <div key={index} className="flex mt-2 justify-start">
            <input
              name="coffeeVarieties"
              defaultValue={varietal.isNew ? varietal.name : varietal}
