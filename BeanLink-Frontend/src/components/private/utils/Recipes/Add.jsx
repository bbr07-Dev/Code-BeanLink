...
export const Add = ({ isOpen, onClose }) => {
  ...
  const { auth } = useAuth();
  //Hook para obtener los datos del formulario
  ...
  //Función que recoje los datos del formulario y hace la petición para crear la nueva receta
  const createRecipe = async (e) => {
    e.preventDefault();
    //Recogemos los datos del formulario
    const params = form;
    try {
      const request = await fetch(Global.url + "recipes/recipe", {
        method: "POST",
  ...
  return (
    isOpen && (
      <div className="fixed inset-0 flex justify-center items-center bg-gray-900/30 backdrop-blur-md transition-all duration-300 z-50">
        <div className="p-8 rounded-md bg-white shadow-md max-w-4xl w-full h-full md:max-h-[70vh] relative overflow-auto">
          <RiCloseCircleFill
            size={30}
            onClick={onClose}
          ...
          <form className="space-y-6" onSubmit={createRecipe}>
            {/* Información básica */}
